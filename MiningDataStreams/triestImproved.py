...
def triest_base( M = 6) :
    ...
    for element in data :
        ...
        if sampleEdge(S,(from_node,to_node), t, M, NumberOfEdges, counter) :
            ...
            # S.add_weighted_edges_from([(int(from_node), int(to_node), int(weight))])
        updateCounters(S,'+', (int(from_node), int(to_node)), t, counter, M)
...
def updateCounters(graf, sign, randomEdge, t, KK_global, M) :
    
    if graf.has_edge(*randomEdge)  :
        (u,v) = randomEdge
...
def test() :
    ...
    print("Traingle count = ", counter[-1])
